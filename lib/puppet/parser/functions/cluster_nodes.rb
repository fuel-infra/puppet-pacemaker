...
Output forms:
* hash - output the hash of the node ids and thir data (used for corosync config template)
* list - output the space ad comma separated nodes and interfaces (used for "pcs cluster setup")
* array - output the plain array of nodes (used for pcmk_auth or "pcs cluster auth")
      eof
  ) do |args|
    nodes = args[0]
    form = args[1] || 'hash'
