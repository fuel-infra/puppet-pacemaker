...
describe pcmk_order('MISSING') do
  before(:each) do
...
describe pcmk_order('p_neutron-dhcp-agent-after-p_neutron-plugin-openvswitch-agent-clone') do
  before(:each) do
    subject.cib = xml_data
  end
...
describe pcmk_order('order-test1-test2-Mandatory') do
  before(:each) do
    subject.cib = xml_data
  end
